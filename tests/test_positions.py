...
class TestRenderPositionSnapshot:
    def test_returns_correct_section_id(self, sample_data: BacktestData, sample_meta) -> None:
        result = render_position_snapshot(sample_data, sample_meta)
        assert result.section_id == "position_snapshot"

    def test_figures_contain_heatmap(self, sample_data: BacktestData, sample_meta) -> None:
        result = render_position_snapshot(sample_data, sample_meta)
...
class TestRenderAttribution:
    def test_returns_correct_section_id(self, sample_data: BacktestData, sample_meta) -> None:
        result = render_attribution(sample_data, sample_meta)
        assert result.section_id == "attribution"

    def test_figures_contain_by_instrument(self, sample_data: BacktestData, sample_meta) -> None:
        result = render_attribution(sample_data, sample_meta)
